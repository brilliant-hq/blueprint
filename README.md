...
For every element, the cycle *design to canonical Blueprint to parse to build to design* MUST be structurally identical for every element type and every persistent property, and a re-emission of the decoded document MUST be byte-identical to the original (idempotent on the second cycle). This is enforced by a standing conformance oracle that compares the full persistent element surface (id, type, name, transform, paint, effects, text, rectangle, circle, and parent data, layout behavior, component and variant and instance data, child-component data, the vector graph, and bounds) and that carries a forcing function: any new persisted codec key that the corpus does not exercise fails the sweep, forcing an emit-and-parse decision.
